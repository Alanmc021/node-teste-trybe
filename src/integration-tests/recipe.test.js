...
describe('3 - Crie um endpoint para o cadastro de receitas', () => {
    ...
    it('Será validado que não é possível cadastrar uma receita com token invalido', async () => {
        await frisby
            .setup({
                request: {
                    headers: {
                        Authorization: '6437658488tokenInvalido',
                        'Content-Type': 'application/json',
                    },
                },
            })
        ...
        await frisby
            ...
            .then((response) => {
                ...
                return frisby
                    ...
                    .expect('status', 201)
                    .then((responseLogin) => {
                        expect(responseLogin._json.result).not.toBeNull();
                        expect(responseLogin._json.result.name).toBe('Frango do jacquin');
                        expect(responseLogin._json.result.ingredients).toBe('Frango');
                        expect(responseLogin._json.result.preparation).toBe('10 min no forno');
                    });
            });
    });
    ...
    describe('7 - Crie um endpoint para a edição de uma receita', () => {
        ...
        it('Será validado que não é possível editar receita sem estar autenticado', async () => {
            ...
            await frisby
                .put(`${url}/recipes/${resultRecipes._id}`, {
    ...
    describe('8 - Crie um endpoint para a exclusão de uma receita', () => {
        ...
        it('Será validado que não é possível excluir receita sem estar autenticado', async () => {
            ...
            await frisby
                .delete(`${url}/recipes/${resultRecipes._id}`)
                .expect('status', 401)
                .then((response) => {
                    const { body } = response;
                    const result = JSON.parse(body);
